...
type Page = "home" | "catalog" | "product" | "cart" | "checkout" | "about" | "contact" | "faq" | "policies" | "login" | "signup";
type NavigateHandler = (page: Page, id?: string, openInNewTab?: boolean) => void;
...
function AuthPage({ mode, onNavigate, onAuthSuccess }: { mode: "login" | "signup"; onNavigate: NavigateHandler; onAuthSuccess?: (user: AuthUser) => void }) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const storedUsers = window.localStorage.getItem("rolexs-users");
    const users = storedUsers ? JSON.parse(storedUsers) as Array<{ name: string; email: string; password: string }> : [];
    ...
    if (isSignup) {
      ...
      const nextUsers = [...users, { name: form.name, email: form.email, password: form.password }];
      window.localStorage.setItem("rolexs-users", JSON.stringify(nextUsers));
      const sessionUser = { name: form.name, email: form.email };
      window.localStorage.setItem("rolexs-session", JSON.stringify(sessionUser));
    ...
    const existingUser = users.find((user) => user.email.toLowerCase() === form.email.toLowerCase() && user.password === form.password);
    if (!existingUser) {
      setMessage("We couldn't find an account matching those details.");
      return;
    }
  ...
  return (
    <section className="min-h-screen px-6 pt-28 pb-16 flex items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(201,168,76,0.16),_transparent_48%)]">
      <div className="w-full max-w-5xl grid gap-8 lg:grid-cols-[1.1fr_0.9fr] items-center">
        <div className="space-y-6">
          <div className="space-y-3">
            <p className="text-[11px] tracking-[0.4em] uppercase text-primary font-mono-label">Rolexs account</p>
            <h1 className="text-4xl md:text-5xl font-semibold tracking-[0.08em] text-foreground">
              {isSignup ? "Create your account" : "Welcome back"}
            </h1>
            <p className="max-w-xl text-base leading-7 text-muted-foreground">
              Access your wishlist, saved preferences, and order history with a single secure account designed for collectors and modern clients.
            </p>
          </div>
          <div className="rounded-3xl border border-border/80 bg-card/80 p-6 shadow-[0_30px_80px_rgba(0,0,0,0.18)] backdrop-blur">
            <form className="space-y-4" onSubmit={handleSubmit}>
              {isSignup && (
                <div>
                  <label className="mb-2 block text-sm text-muted-foreground" htmlFor="name">Full name</label>
                  <input
                    id="name"
                    value={form.name}
              ...
              <div>
                <label className="mb-2 block text-sm text-muted-foreground" htmlFor="email">Email address</label>
                <input
                  id="email"
                  type="email"
                  value={form.email}
              ...
              {isSignup && (
                <div>
                  <label className="mb-2 block text-sm text-muted-foreground" htmlFor="confirmPassword">Confirm password</label>
                  <input
                    id="confirmPassword"
                    type="password"
            ...
            <div className="mt-4 text-sm text-muted-foreground">
              {isSignup ? "Already have an account?" : "New to Rolexs?"}{" "}
              <button className="text-primary hover:underline" onClick={() => onNavigate(isSignup ? "login" : "signup")}> 
                {isSignup ? "Log in instead" : "Create one"}
              </button>
            </div>
          </div>
        </div>
        <div className="rounded-[2rem] border border-border/80 bg-background/70 p-8 shadow-[0_30px_90px_rgba(0,0,0,0.2)]">
          <p className="text-[11px] tracking-[0.4em] uppercase text-primary font-mono-label">Why join</p>
          <ul className="mt-6 space-y-4 text-sm text-muted-foreground">
            <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-primary" />Personalized watch recommendations based on your style.</li>
            <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-primary" />Fast checkout with saved details and preferred delivery.</li>
            <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-primary" />Direct access to concierge support and order updates.</li>
          </ul>
        </div>
      </div>
    </section>
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <div className="hidden md:flex items-center gap-3">
            <button
              ...
              className="text-[11px] tracking-[0.2em] uppercase text-muted-foreground hover:text-foreground transition-colors font-mono-label"
            >
              Login
            </button>
            <button
              onClick={() => onNavigate("signup")}
              className="rounded-full border border-primary/60 px-4 py-2 text-[11px] tracking-[0.2em] uppercase text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
            >
              Sign Up
            </button>
          </div>
          ...
          >
            <div className="px-6 py-4 flex flex-col gap-4">
              {navLinks.map((l) => (
                <button
                  key={l.page}
                  onClick={() => { onNavigate(l.page, undefined, true); setOpen(false); }}
              ...
              <button
                onClick={() => { onNavigate("signup"); setOpen(false); }}
                className="text-left text-[11px] tracking-[0.2em] uppercase text-foreground hover:text-primary transition-colors font-mono-label py-1"
              >
                Sign Up
              </button>
            ...
            <div key={col.title}>
              ...
              <ul className="space-y-2">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <button
                      onClick={() => onNavigate(l.page, undefined, true)}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pb-20 w-full">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <button
                onClick={() => onNavigate("catalog", undefined, true)}
              ...
              <button
                onClick={() => onNavigate("about", undefined, true)}
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                {[
                  ...
                  <button
                    key={option.id}
                    type="button"
                    onClick={() => setPaymentMethod(option.id as "card" | "bank" | "upi")}
              ...
              {paymentMethod === "bank" && (
                <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">
                  Please transfer the amount to our Geneva bank account. We will confirm your order once the payment is received.
                </div>
              )}
              {paymentMethod === "upi" && (
                <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">
                  Scan the UPI QR code or use your preferred UPI app to complete the payment instantly.
                </div>
              )}
            </div>
          )}
...
function FAQPage({ onNavigate }: { onNavigate: NavigateHandler }) {
...
function AppContent() {
  ...
  const [authUser, setAuthUser] = useState<AuthUser | null>(() => {
    ...
    const storedSession = window.localStorage.getItem("rolexs-session");
    if (!storedSession) return null;
    try {
      return JSON.parse(storedSession) as AuthUser;
    } catch {
      window.localStorage.removeItem("rolexs-session");
      return null;
    }
  });
  ...
  const navigateToPage: NavigateHandler = (p, id, openInNewTab) => {
    if ((p === "cart" || p === "checkout") && !authUser) {
      showToast("Please sign up or log in to continue shopping.");
      navigate("/signup");
      return;
    }

    const path = getPathForPage(p, id);
              ...
              element={
                authUser ? (
                  ...
                  <motion.div key="login" {...fade}>
                    <AuthPage mode="login" onNavigate={navigateToPage} onAuthSuccess={setAuthUser} />
                  </motion.div>
                )
              }
            />
              ...
              element={
                authUser ? (
                  ...
                  <motion.div key="signup" {...fade}>
                    <AuthPage mode="signup" onNavigate={navigateToPage} onAuthSuccess={setAuthUser} />
                  </motion.div>
                )
              }
            />
